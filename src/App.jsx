...
function Login({ onRegister, onSuccess, onError }) { const [form, setForm] = useState({ email: '', motDePasse: '' }); const [busy, setBusy] = useState(false); async function submit(event) { event.preventDefault(); setBusy(true); onError(''); try { const result = await api.auth.login(form); onSuccess(result.utilisateur) } catch (error) { onError(`Connexion impossible : ${error.message}`) } finally { setBusy(false) } } return <AuthLayout mode="login" onSwitch={onRegister}><form onSubmit={submit}><label>Email<input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required /></label><label>Mot de passe<input type="password" value={form.motDePasse} onChange={(e) => setForm({ ...form, motDePasse: e.target.value })} required /></label><button className="primary-button" disabled={busy}>{busy ? 'Connexion...' : 'Se connecter'}</button></form></AuthLayout> }
function Register({ onLogin, onSuccess, onError }) { const [form, setForm] = useState(emptyRegistration); const [busy, setBusy] = useState(false); const update = (event) => setForm({ ...form, [event.target.name]: event.target.value }); async function submit(event) { event.preventDefault(); setBusy(true); onError(''); try { const { etudiant, ...values } = form; const result = await api.auth.register({ ...values, role: etudiant ? 'ETUDIANT' : 'PROF' }); onSuccess(result.utilisateur) } catch (error) { onError(`Inscription impossible : ${error.message}`) } finally { setBusy(false) } } return <AuthLayout mode="register" onSwitch={onLogin}><form onSubmit={submit}><label>Matricule<input name="matricule" value={form.matricule} onChange={update} maxLength="10" required /></label><div className="form-grid"><label>Nom<input name="nom" value={form.nom} onChange={update} required /></label><label>Prénom<input name="prenom" value={form.prenom} onChange={update} required /></label></div><label>Email<input name="email" type="email" value={form.email} onChange={update} required /></label><label>Mot de passe<input name="motDePasse" type="password" minLength="8" value={form.motDePasse} onChange={update} required /></label><button className="primary-button" disabled={busy}>{busy ? 'Création...' : 'Créer le compte'}</button></form></AuthLayout> }
...
export default function App() {
  const [screen, setScreen] = useState(() => window.location.pathname === '/study/comments' ? 'comments' : window.location.pathname === '/login' ? 'login' : 'study'); const [user, setUser] = useState(() => { try { return JSON.parse(localStorage.getItem('user')) } catch { return null } }); const [menu, setMenu] = useState(false); const [account, setAccount] = useState(false); const [error, setError] = useState('')
  function navigate(path) { window.history.pushState({}, '', path); setMenu(false); setScreen(path === '/study/comments' ? 'comments' : 'study') }
